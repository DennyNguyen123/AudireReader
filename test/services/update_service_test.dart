...
import 'package:novel_reader/services/update_service.dart';
...
void main() {
  group('UpdateService.isNewerVersion', () {
    ...
    test('TC04 — minor dạng số lớn: 1.9.0 vs 1.10.0 (10 > 9, không phải so sánh string)', () {
  ...
  group('UpdateService.getDownloadUrl', () {
    ...
    const fallbackUrl = 'https://github.com/${UpdateService.owner}/${UpdateService.repo}/releases/latest';
    ...
    test('TC11 — có file .apk: trả về APK URL trên Android, fallback trên các nền tảng khác', () {
      final assets = [
        {'name': 'app.apk', 'browser_download_url': 'http://example.com/app.apk'},
    ...
    test('TC12 — có file .ipa: trả về IPA URL trên iOS, fallback trên các nền tảng khác', () {
      final assets = [
        {'name': 'app.ipa', 'browser_download_url': 'http://example.com/app.ipa'},
      ];
      final result = UpdateService.getDownloadUrl(assets);
      if (Platform.isIOS) {
